...
export function buildBottomNavHTML(opts: BottomNavOptions): string {
  ...
  const settingsHTML = opts.showSettings === false
    ? ''
    : `<button class="bn-btn" data-view="settings" data-action="show-view">
  ...
  if (pages.length === 1) {
    ...
    pagesHTML = `<button class="bn-btn" data-view="${viewId}" data-action="show-view">
    ...
    const items = pages.map(p => {
      ...
      return `<button type="button" class="bn-sheet-item" data-view="${viewId}" data-action="show-view" data-close-sheet="true">${escapeHTML(p.name)}</button>`;
    }).join('');
    pagesHTML = `<button class="bn-btn" data-action="toggle-bottom-sheet">
  ...
  return `<div class="hdp-bottom-nav">
    <button class="bn-btn bn-btn--home" data-view="home" data-action="show-view">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
      <span>首页</span>
    </button>
    <button class="bn-btn" data-view="devices" data-action="show-view">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></svg>
      <span>设备</span>
    </button>
    ${pagesHTML}
    ${settingsHTML}
    <button class="bn-btn" data-action="toggle-ha-menu">
