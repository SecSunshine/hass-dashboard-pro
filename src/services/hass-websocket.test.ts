...
describe('hass websocket script', () => {
  ...
  it('does not delegate-toggle clicks inside domain-specific cards', () => {
    ...
    expect(js).toContain("if (action === 'cover-position' || action === 'media-volume' || action === 'number-set' || action === 'select-option') return false;");
    ...
    expect(js).toContain('function hdpSetSelectOption(entityId, option)');
    expect(js).toContain("domain !== 'select' && domain !== 'input_select'");
    expect(js).toContain("hdpCallEntityService(hass, domain, 'select_option'");
